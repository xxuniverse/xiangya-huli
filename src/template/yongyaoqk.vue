  ...
  <div>
    <div class="site">
      <p v-for="(item,index) in choseArr"
         ...
         :key="index"
         style="margin-left:100rpx;color:#3CA1FF">{{item.val}}</p>
      <div v-if="choseArr.length<=0">
        <checkbox-group @change="checkboxChange($event)"
          ...
          <label class="checkbox"
            ...
            <input style="position: absolute;top: -1px;left: 81px;width: 102px;border-bottom: 1px solid #e6e6e6;z-index: 999;"
...
export default {
  ...
  methods: {
    ...
    choose() {
      ...
      if (this.choseArr.length > 0) {
        this.$emit("checkboxChange", { name: "drugType", data: this.choseArr });
      }
    },
    checkboxCancel() {
      this.choseArr = [];
  ...
  mounted() {
    if (this.oldData && Object.keys(this.oldData).length > 0) {
      let id2 =
        this.oldData.drugType &&
        JSON.parse(this.oldData.drugType).map(el => {
          if (el.id == 64) {
            this.temp = el.val;
          }
          return String(el.id);
